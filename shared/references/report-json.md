...
{
  "doc": {
    "title": "标题", "headerLeft": "页眉左", "headerRight": "成品用途标识",
...
页眉、用途与表头按已选内部/客户版本填写，不默认“内部讨论稿”。客户表可用读者可理解的比较标题；
保留 module 和 acknowledgementColumn 供结构核验，不代表正文必须显示“攻防”“需要先承认的”等训练标签。
...
完整产品推广报告设置 `minQuestions: 12`；通用检查器未指定时的旧默认为十问，其他任务按范围填写。
这些数字只是结构要求，不证明回答质量。客户正文与制作人说明分别构建，不把过程说明并入客户文件。
