...
description: 为基金、ETF及其他有资料支持的产品制作中文研究与推广报告，或优化已有材料。先确认范围与叙事，再提炼核心判断、一句话定位、关键问答和话术；支持完整报告、部分模块、现稿优化，不套用固定行业口号。可单独完成研究、定位与销售材料；纯排版可用cn-docx-report。本技能不用于个人升职申请。Also use for evidence-backed product promotion, positioning and sales enablement in Chinese.
---
...
- 先写准确的定位底稿，再给传播候选与推荐；允许直白表达，不强制比喻。十二问十二答按真实决策障碍排序，短答直接回应，依据与边界可放内部注释；最难的几题追加追问压力测试，详见 `references/toolkit-templates.md`。
...
当前版本 0.1.0-preview.1（2026-09-25），独立项目 Promotion Master，源自 report-skills 1.3.0。预览版待使用者验收；不代表已获对外使用批准。四个卫星可独立使用，主入口本身自包含。共享文件修改 shared/ 后同步。
