...
发现旧版卫星时继续使用本包规则并说明，不修改用户安装、不猜路径。各模块共享 F/R/C 与确认记录，不重复立项。
...
当前版本 0.1.0-preview.2（2026-09-25），独立项目 Promotion Master，源自 report-skills 1.3.0。支持内部版、客户版与两版配套；仍为预览，不代表已获对外使用批准。四个卫星可独立使用，主入口本身自包含。共享文件修改 shared/ 后同步。
