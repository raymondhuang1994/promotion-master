#!/usr/bin/env bash
# 默认安装文本工作流；--with-word 准备渲染依赖，--all 额外安装卫星。
set -euo pipefail
...
usage() {
  echo "用法：tools/install_codex.sh [--all] [--with-word] [--dest 目标目录 | 目标目录]"
  echo "  默认：只安装 promotion-master 文本工作流；无需 Node/npm 或 Word 工具，不下载依赖。"
  echo "  --all：同时安装仓库内卫星；同名目录或其他仓库的链接冲突时停止。"
  echo "  --with-word：安装本地 Node 依赖并检查 Word 渲染工具；缺少系统依赖时停止，不创建链接。"
...
if ! command -v python3 >/dev/null; then echo "缺少 python3，请先安装；尚未创建技能链接。" >&2; exit 1; fi
...
check_target() {
  ...
  if [ -L "$target" ]; then
    if ! [ "$target" -ef "$source" ]; then
      ...
      echo "未替换已有链接。请用 --dest 指定独立目录，或先自行确认并处理冲突。" >&2
      return 1
    fi
  elif [ -e "$target" ]; then
...
if [ "$WITH_WORD" -eq 1 ]; then
  for tool in node npm; do
    if ! command -v "$tool" >/dev/null; then echo "--with-word 缺少 ${tool}，请先安装；尚未创建技能链接。" >&2; exit 1; fi
  done
  if [ ! -f "$ROOT/skills/promotion-master/scripts/package.json" ] || [ ! -f "$ROOT/skills/promotion-master/scripts/build.js" ] || [ ! -f "$ROOT/shared/scripts/doctor.py" ]; then
    echo "缺少 Word 渲染入口、依赖清单或预检脚本；请重新下载完整仓库。" >&2; exit 1
  fi
...
if [ "$WITH_WORD" -eq 1 ]; then
  echo "Word 工具链预检完成；版式交付仍须逐页目视检查。"
else
  echo "文本模式：未下载或检查 Word 渲染依赖；需要生成 Word 时，可用相同选项加 --with-word 再运行。"
fi
